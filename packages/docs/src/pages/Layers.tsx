...
function Introduction() {
  return (
    <Stack as={Section} spacing="space-lg">
      <Text>
        Layers are a system of components and functions that allow for rendering content outside the
        normal flow of a page. The system is composed of 3 types of layers, each representing a
        different level of relevance to whatever element owns it. <code>Tooltip</code> is
        immediately attached to the target element, only showing when the target is being interacted
        with, and immediately disappearing afterward. <code>Popout</code> is one level higher, still
        attaching itself to a target element, but staying visible until the user performs another
        action to close it. And finally <code>Modal</code> is completely detached from it's source
        and appears as a full-screen takeover that must be interacted with before the rest of the
...
function PopoutComponent() {
  ...
  return (
    <Stack as={Section} spacing="space-lg">
      <Header tag="h2">Popout</Header>
      <Text>
        <code>Popout</code> is a contextual layer meant to provide additional information or details
        about some entity without taking up more space in the page flow. Unlike <code>Tooltip</code>
        , which is only visible as a user hovers the target, <code>Popout</code> is best for
        semi-permanent information that the user may want to interact with or keep visible as they
        continue to browse the rest of the page.
      </Text>
      <Card>
      ...
      <Text>
        If the popout is too large to fit within the viewport with the requested <code>attach</code>{" "}
        and <code>align</code> props, it will try to automatically invert and nudge the popout into
        a position that will fit. This can result in the popout not respecting the requested values,
        but ensures content isn't cut off when it could otherwise be visible.
      </Text>
    </Stack>
  );
}
...
function TooltipComponent() {
  ...
  return (
    <Stack as={Section} spacing="space-lg">
      ...
      <Text>
        <code>Tooltip</code> is the lowest-level layer for providing supplemental information to the
        target element. They are only visible when the user is directly interacting with the target,
        either by hovering it with a pointer or focusing it with a keyboard.
      </Text>
      <Card>
      ...
      <Text>
        As such an ephemeral layer, <code>Tooltip</code> should only be used for immediately-useful
        information, like a name for an icon, or a quick descriptive sentence to expand on a single
        word, and should <em>not</em> be used for any kind of interaction.
      </Text>
      <Text>
