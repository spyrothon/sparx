  ...
  return (
    <Stack as={Section} spacing="space-lg">
      ...
      <Text>
        ...
        <code>Text</code> renders at 18px with Encode Sans, in a high-contrast color for the theme's
        primary background.
      </Text>
      ...
      <Text>
        <code>Text</code> can also use the style variants that <code>Header</code> normally uses to
        achieve the same styles without the semantic implications of a header tag. This is good for
        hierarchical combinations of text, like a field name and a field value appearing next to
        each other, to indicate the distinction between the two.
      </Text>
      <Stack as={Card} direction="vertical" spacing="space-md">
        <div>
      ...
      <Text>
        Each font and size variation also has a number of color options, denoted in the variant name
        as <code>name/color</code>. <code>normal</code> and <code>secondary</code> are the most
        common colors across all instances of text, but the additional status colors can also be
        used to draw attention to a state in a way that is more subtle than a <code>Card</code> or{" "}
        <code>Callout</code> might be.
      </Text>
      <Card>
      ...
      <Text>
        In some cases, the desired color for a <code>Text</code> element isn't known ahead of time,
        or is dependent on a parent component, like inside of a <code>Hero</code> block. In these
        cases, an <code>inherit</code> option is alo available for all variants to have the
  ...
  return (
    <Stack as={Section} spacing="space-lg">
      ...
      <Text>
        ...
        semantic structure. Secondly, <code>Header</code> uses a bold font weight in Overlock rather
        than Encode Sans to distinguish it from normal text regardless of styling.
      </Text>
      <Text>
        ...
        <code>Text</code> components. This should only be used to maintain a visual style as well as
        the semantic information of a header tag. Otherwise, prefer using <code>Text</code> instead.
      </Text>
      <Stack as={Card} spacing="space-md">
      ...
      <Text>
        <code>Header</code> also provides a separate prop, <code>uppercase</code>, to automatically
        transform the given text to ALL CAPS. This should be used sparingly, and only for
        short-length text, as longer text quickly becomes hard to read. Most often,{" "}
  ...
  return (
    <Stack as={Section} spacing="space-lg">
      ...
      <Text>
        ...
        more content, or triggering a change in state should probably use <code>Clickable</code> or{" "}
        <code>Button</code> instead.
      </Text>
    </Stack>
  );
}
